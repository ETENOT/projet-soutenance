                                    ...
                                    <form class="needs-validation" novalidate method="POST" action="{{ route('register') }}">
                                        ...
                                        <div id="particulier-fields">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <label for="telephone" class="form-label">Téléphone <span class="text-danger">*</span></label><br>
                                        ...
                                        <div id="entreprise-fields" class="d-none">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                {{--
                                                    ...
                                                    Même principe que #telephone plus haut : champ VISIBLE sans

                                                    "name" (jamais soumis tel quel), initialisé par intl-tel-input

                                                    (cf. @section('script') plus bas) pour le sélecteur de pays,

                                                    l'adaptation du placeholder/de la longueur à l'indicatif, et la

                                                    validation. "disabled" par défaut car ce bloc "entreprise" est

                                                    masqué tant que le rôle "particulier" est sélectionné.

                                                --}}

                                                <input type="tel" class="form-control @error('contact_principal') is-invalid @enderror"

                                                    id="contact_principal" required disabled>

                                                {{-- Un seul bloc "invalid-feedback" : voir l'explication détaillée

                                                     sur le champ #telephone plus haut, même raisonnement ici. --}}
                                                ...
                                                <div class="invalid-feedback">
                                                    ...
                                                    @else
                                                        ...
                                                        Veuillez entrer un numéro de contact valide pour le pays sélectionné.
                ...
                <div class="row">
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="text-center">
                            ...
                            <p class="mb-0">© <script>document.write(new Date().getFullYear())</script> Formation NéoVisions</p>
...
@section('script')
    ...
    <script>
        ...
        // un champ téléphone, avec adaptation automatique du placeholder et

        // de la validité au nombre de chiffres attendu pour le pays choisi.

        //

        // Factorisé ici car appliqué à DEUX champs indépendants du

        // formulaire : #telephone (bloc particulier) et #contact_principal

        // (bloc entreprise) — chacun a sa propre instance intl-tel-input,

        // son propre pays sélectionné, sa propre validité.

        // ------------------------------------------------------------------

        function setupPhoneField(inputId, oldValue) {
            ...
            const iti = window.intlTelInput(inputEl, {
                ...
                // Noms de pays affichés en français dans la liste ("Allemagne" et

                // non "Germany") via l'API native Intl.DisplayNames du navigateur.

                countryNameLocale: 'fr',

                // Affiche l'indicatif (+241) à côté du drapeau, non modifiable

                // directement dans le champ de saisie (évite qu'il tape "+241" à

                // la fois dans l'indicatif ET dans le numéro).
                ...
                // Le champ visible n'a pas de "name" : rien n'est donc envoyé au

                // serveur depuis lui directement. hiddenInputs crée automatiquement

                // (au moment du submit du formulaire) un <input type="hidden"

                // name="{inputId}"> contenant le numéro complet au format

                // international (ex. "+24177123456"), qui est ce que reçoit

                // RegisteredUserController@store dans $request->telephone ou

                // $request->contact_principal selon le champ.

                hiddenInputs: () => ({ phone: inputId }),
            ...
            // AUTRE champ (ex. mot de passe non confirmé), oldValue contient le

            // numéro complet au format international précédemment soumis (celui

            // du hidden input généré par hiddenInputs ci-dessus). iti.setNumber()

            // sait parser ce format, retrouver automatiquement le bon pays ET

            // ré-afficher le numéro en format national dans le champ.
            ...
            // setCustomValidity) dès que le numéro tapé ne correspond pas au

            // format attendu pour le pays sélectionné. Combiné à "needs-validation"
            ...
            function updateValidity() {
                ...
                inputEl.setCustomValidity(valid === false
            ...
            // l'utilisateur change de pays dans la liste déroulante (sans

            // forcément retoucher au numéro) — la validité doit être réévaluée

            // immédiatement, puisque la longueur attendue change avec le pays.

            inputEl.addEventListener('countrychange', updateValidity);



            return { inputEl, iti, updateValidity };

        }



        const telephoneField = setupPhoneField('telephone', @json(old('telephone')));

        const contactPrincipalField = setupPhoneField('contact_principal', @json(old('contact_principal')));
        ...
        function toggleRoleFields() {
            ...
            //    #telephone et #contact_principal sont exclus de ces boucles

            //    génériques : ils sont pilotés par iti.setDisabled() juste après,

            //    qui désactive à la fois le champ ET le bouton drapeau/sélecteur

            //    de pays d'intl-tel-input (un simple ".disabled = true" sur

            //    l'input laisserait le bouton drapeau cliquable alors que le

            //    champ à côté serait grisé).
            ...
            entrepriseBlock.querySelectorAll('input:not(#contact_principal)').forEach(input => {
            ...
            telephoneField.iti.setDisabled(!isParticulier);

            telephoneField.updateValidity();



            contactPrincipalField.iti.setDisabled(!isEntreprise);

            contactPrincipalField.updateValidity();
